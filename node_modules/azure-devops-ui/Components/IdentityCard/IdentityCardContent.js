    ...
    IdentityCardContent.prototype.renderCard = function () {
        ...
        return innerCard ? (React.createElement(FocusZone, { circularNavigation: true, defaultActiveElement: ".bolt-identity-card-focus-element", direction: FocusZoneDirection.Vertical, focusOnMount: true, handleTabKey: true },
            ...
            React.createElement("div", { className: cssClassName, onKeyDown: this.onKeyDown },
                isAdOrAadOrGroup && headerIdentity && React.createElement(HeaderElement, { identity: headerIdentity, onClickFunction: this.props.onHeaderClick, imageUrlResolver: this.props.imageUrlResolver }),
        ...
        if (isGroup) {
            switch (this.props.dataProps.cardType) {
                case CardType.Organization:
                    return React.createElement(GroupMembersCard, { identity: identity, members: successors, onClickEntity: this.props.onClickEntity, imageUrlResolver: this.props.imageUrlResolver });
            ...
            return React.createElement(GitHubCard, { identity: identity, imageUrlResolver: this.props.imageUrlResolver });
        }
        else if (isVsdUser) {
            return React.createElement(DefaultAbridgedCard, { identity: identity, imageUrlResolver: this.props.imageUrlResolver });
        }
        else if (isWmdUser) {
            return React.createElement(DefaultSimpleCard, { identity: identity, imageUrlResolver: this.props.imageUrlResolver });
        }
        else if (isAadSp) {
            return React.createElement(ServicePrincipalCard, { identity: identity, imageUrlResolver: this.props.imageUrlResolver });
        }
        else if (isAadUser || isAdUser) {
            switch (this.props.dataProps.cardType) {
                case CardType.Contact:
                    return React.createElement(ContactCard, { identity: identity, imageUrlResolver: this.props.imageUrlResolver });
                case CardType.Organization:
                    return (React.createElement(OrganizationCard, { identity: identity, managerList: managerList, directReportList: directReportList, onClickEntity: this.props.onClickEntity, imageUrlResolver: this.props.imageUrlResolver }));
                case CardType.Default:
                default:
                    var manager = managerList && managerList.length > 0 ? managerList[managerList.length - 1] : undefined;
                    return (React.createElement(DefaultCard, { identity: identity, manager: manager, isPreviousHeader: !!this.props.dataProps.previousDataState || !!this.props.initialHeader, showContactCard: this.props.onShowContactCard, showOrganizationCard: this.props.onShowOrganizationCard, onClickEntity: this.props.onClickEntity, imageUrlResolver: this.props.imageUrlResolver }));
            }
        }
